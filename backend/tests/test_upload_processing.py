...
def _resolve_email_fixture() -> tuple[Path, str]:
    data_source_dir = _repo_root() / "docs" / "data_source"
...
def _login_headers(client: TestClient, login_id: str = "Editor1", password: str = "editor123") -> dict[str, str]:
...
class UploadProcessingTests(unittest.TestCase):
    ...
    def test_upload_text_file_creates_processed_draft(self) -> None:
        sample_path = _repo_root() / "docs" / "data_source" / "Error_Code_AUTH_401.txt"
        ...
        payload = _poll_processing(self.client, upload_response["processing_id"], self.headers)
    ...
    def test_upload_duplicate_file_is_flagged(self) -> None:
        ...
        payload = _poll_processing(self.client, response.json()["processing_id"], self.headers)
    ...
    def test_upload_real_image_uses_easyocr(self) -> None:
        sample_path = _repo_root() / "docs" / "data_source" / "Teams_Message_2.jpg"
